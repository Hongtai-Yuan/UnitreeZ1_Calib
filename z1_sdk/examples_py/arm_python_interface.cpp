...
{
public:
    ArmInterface(bool hasGripper):unitreeArm(hasGripper){};//ArmInterface为在python中的类名，unitreeArm为C++中的类名
    ~ArmInterface(){};
    void loopOn() { sendRecvThread->start();}//LoopFunc的start()
    void loopOff() { sendRecvThread->shutdown();}//LoopFunc的shutdown()
    void setFsmLowcmd()
    {
    ...
    {
        return _ctrlComp->recvState.state;//结构体recvstate有三种变量，state为ArmFSMState、JointState为jointState[7]、Posture cartesianState;
    }
};
...
PYBIND11_MODULE(unitree_arm_interface, m){
    ...
    m.def("postureToHomo", &postureToHomo);//homo为齐次矩阵的意思
